...
for ii=1:1:basesize,
    template_source = [pth '\wROI_MNI_V4.nii'];
    template_image = [proc_dir '\wROI_MNI_V4.nii'];
    ...
    reslice_temp = cell(2,1);
    if sum(dim_test)~=3,
        reslice_temp{1,:} = [proc_dir '\' basename baseext ',1'];
        reslice_temp{2,:} = [proc_dir '\' tempname tempext ',1'];
...
%         copyfile(template_image,temp_name,'f');
        ...
        resliceflags = struct('interp',1,'mask',1,'mean',0,'which',1,'wrap',[0 0 0]',...
            'prefix','r');
        disp('Reslicing Template...');
        spm_reslice({reslice_temp},resliceflags);
        
        delete(template_image);
        clear template_hdr template_img template_image
        template_image = [proc_dir '\r' tempname tempext];
        template_hdr = spm_vol(template_image);
        template_img = spm_read_vols(template_hdr);
    end
