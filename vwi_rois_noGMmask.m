...
%% Prompt for the number of subject's to analyze.
box = ('How many subjects to analyze:');
box_title = 'ROI';
num_lines = 1;
default = {'1'};
SubNum = inputdlg(box,box_title,num_lines,default);
subint = round(str2double(SubNum{1}));
while isnan(subint)
    msg = ('A number must be entered:');
    uiwait(msgbox(msg,'ROI'));
    SubNum = inputdlg(box,box_title,num_lines,default);
    subint = round(str2double(SubNum{1}));
    clear msg
end
clear box default subint
SubNum = SubNum{1};

%% Prompt for subject numbers and their study name.
for ii=1:str2double(SubNum),
    msg1 = ('Please enter subject number and study name of subject #');
    msg = sprintf('%s%d',msg1,ii);
    uiwait(msgbox(msg,box_title));
    Study_Sub;
    waitfor(Study_Sub);
    sub = evalin('base','sub');
    study = evalin('base','study');
    eval(sprintf('study_%d = study;',ii));
    eval(sprintf('sub_%d = sub;',ii));
    clear msg1 msg sub study 
end

%% Read Study Protocol and make subject directories
for ii=1:str2double(SubNum),
    study = eval(sprintf('study_%d',ii));
    sub = eval(sprintf('sub_%d',ii));
    [~,~,raw]=xlsread([pth '\Studies\' study '.xlsx'],'Study-Protocol');
    studyprotocol = raw;
    clear raw;
    study_dir = studyprotocol{1,3};
    sub_dir = [study_dir '\ROI\' sub];
    if exist(sub_dir,'dir') == 0;
        mkdir(sub_dir);
    end
    eval(sprintf('sub_dir_%d = sub_dir;',ii));
    clear sub_dir study sub raw studyprotocol study_dir
end

%% Prompt to select subject's coreg_seg processing directory and copy data
for ii=1:1:str2double(SubNum);
    ...
    msg1 = ('Please select ');
    msg2 = ('''s Processing folder.');
    msg = sprintf('%s%s%s',msg1,sub,msg2);
    uiwait(msgbox(msg,box_title));
    procdir = uigetdir(study_dir,msg);
    copyfile(procdir,sub_dir);
    clear msg msg1 msg2 sub study study_dir procdir
end
...
if Group == 1,
    ...
    for zz = 1:1:str2double(SubNum);
        sub_dir = eval(sprintf('sub_dir_%d',zz));
        ROI_MNI_dir = [sub_dir '\ROI\ROI_MNI\'];
        mkdir(ROI_MNI_dir);
        for ii = 1:1:size(Selection,2),
            roitext = textread([rois_home '\' num2str(Selection(ii)) '.txt'],'%s');
            roitextsize = size(roitext,1);
            for jj = 1:1:roitextsize
                ROI_home = [pth '\ROIs\' roitext{jj}];
...
%% Coregister and Segment
roi_coreg_seg(sub,study);
...
for ii=1:1:str2double(SubNum);
    ...
    % copy segmented tissue images into ROI dir
    segvol = dir([sub_dir, '\p*.nii']);
    if size(segvol,1) == 0,
        segvol = dir([sub_dir, '\c*.nii']);
    end
    segvol_name = {segvol.name};
    segvol_name = str2mat(segvol_name);
    segvol_size = size(segvol_name,1);
    for kk=1:1:segvol_size
        seg_dir = [sub_dir '\' segvol_name(kk,:)];
        copyfile(seg_dir,ROI_dir);
        clear seg_dir
    end
...
%     seg_name = seg_dir.name;
    ...
    % remove segmented partitions from ROI dir  %%%%%%%%%%%%%%%%%%%%%
    segvol = dir([ROI_dir, '\p*.nii']);
    if size(segvol,1) == 0,
        segvol = dir([sub_dir, '\c*.nii']);
    end
    segvol_name = {segvol.name};
    segvol_name = str2mat(segvol_name);
    segvol_size = size(segvol_name,1);
    for kk=1:segvol_size
        rseg_dir = [ROI_dir segvol_name(kk,:)];
        newseg_dir = [sub_dir '\seg_' segvol_name(kk,:)];
        movefile(rseg_dir,newseg_dir);
    end
...
for ii=1:1:str2double(SubNum);
    ...
    MRI_dir = dir([sub_dir, '\r*.nii']);
    MRI_name = MRI_dir.name;
    GM_dir = dir([sub_dir, '\seg_p1*.nii']);
    if size(GM_dir,1) == 0,
        GM_dir = dir([sub_dir, '\seg_c1*.nii']);
    end
    GM_name = GM_dir.name;
    WM_dir = dir([sub_dir, '\seg_p2*.nii']);
    if size(WM_dir,1) == 0,
        WM_dir = dir([sub_dir, '\seg_c2*.nii']);
    end
    WM_name = WM_dir.name;
    CSF_dir = dir([sub_dir, '\seg_p3*.nii']);
    if size(CSF_dir,1) == 0,
        CSF_dir = dir([sub_dir, '\seg_c3*.nii']);
    end
